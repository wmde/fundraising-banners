...
import Banner from '@banners/desktop/C24_WMDE_Desktop_DE_19/components/BannerVar.vue';
...
import { useReasonsToDonateFeaturesUsingTenGoodReasonsSticker, useReasonsToDonateScrollFeaturesUsingTenGoodReasonsSticker } from '@test/features/ReasonsToDonate';
...
describe( 'BannerVar.vue', () => {
	...
	describe( 'Reasons to Donate', () => {
		test.each( [
			...
			await useReasonsToDonateFeaturesUsingTenGoodReasonsSticker[ testName ]( getWrapper(), tracker );
		} );

		test.each( [
			[ 'expectScrollsToFormWhenCallToActionIsClickedUsingTenGoodReasonsSticker' ]
		] )( '%s', async ( testName: string ) => {
			await useReasonsToDonateScrollFeaturesUsingTenGoodReasonsSticker[ testName ]( getWrapper(), pageScroller );
		} );
	} );
