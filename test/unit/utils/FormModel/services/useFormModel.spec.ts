...
describe( 'useFormModel', () => {
	...
	it( 'disables all recurring intervals if payment method is SOFORT', async function () {
		const model = useFormModel();
	...
	it( 'allows all intervals for any other payment method than SOFORT', async function () {
		const model = useFormModel();
	...
	it( 'disables SOFORT when the interval is set an recurring interval', async function () {
		const model = useFormModel();
	...
	it( 'allows all payment methods on one time interval', async function () {
		const model = useFormModel();
	...
	it( 'disables anonymous address type when direct debit was selected', async function () {
		const model = useFormModel();
		model.paymentMethod.value = PaymentMethods.DIRECT_DEBIT.value;
	...
	it( 'allows all address types when any other payment type than direct debit was selected', async function () {
		const model = useFormModel();
