...
2. Copy the colours from the `variables/_colors.scss` file into the `color_light.scss` and delete `variables/_colors.scss`. Deleting the old file first causes the compiler to throw errors which makes it a little easier to find all the usages.
