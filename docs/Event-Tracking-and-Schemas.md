...
This separation requires two separate implementations, one for each external service. 
...
appropriate tracking code and sent to the external service.

In the banner folders you will find an `event_map.ts` file that handles mapping the internal events to external ones.
...
Our schemas consist of 2 parts, the base schema and fragments. Fragments are small sub-schemas that are included by and make up base schemas.
Think of them like includes, our base schema is made up of various fragments.
...
2. Create a common.yaml file and edit it according to the [official documentation](https://wikitech.wikimedia.org/wiki/Event_Platform/Schemas#Creating_a_new_schema).
3. When the common.yaml file is ready run `npm run build-new /path/to/mt/common.yaml` and it will generate the other schema files for you.

**Modify an existing Schema or Fragment**

1. Modify the common.yaml file in the schema you need to change.
2. Make sure to bump the version number of the schema
3. When the common.yaml file is ready run `npm run build-new /path/to/mt/common.yaml` and it will generate the other schema files for you. These files will have the new version number meaning backwards compatibility will not be broken, and old banners can still use the old version.
