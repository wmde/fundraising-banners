...
import MinimisedBanner from '@banners/desktop/C24_WMDE_Desktop_DE_14_minimised/components/MinimisedBanner.vue';
import { useScrollMinimiser } from '@banners/desktop/C24_WMDE_Desktop_DE_14_minimised/useScrollMinimiser';
...
const wasMinimisedByDonor = ref<boolean>( false );

watch( contentState, async () => {
	//emit( 'bannerContentChanged' );
} );

useScrollMinimiser( 500, () => {
	if ( wasMinimisedByDonor.value ) {
		return;
	}

	if ( contentState.value === ContentStates.Main ) {
	...
	if ( contentState.value === ContentStates.Minimised && !wasMinimisedByDonor.value ) {
...
function onMinimiseBanner(): void {
	...
	wasMinimisedByDonor.value = true;
	emit( 'bannerContentChanged' );
}
